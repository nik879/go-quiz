...
func GetSpecificQuestion(questionID int) (question Question, err error) {
	db := migration.GetDbInstance()
	specificQuestionQuery, err := db.Query("SELECT * FROM questions WHERE ID=?;", questionID)
	defer specificQuestionQuery.Close()
	if err != nil {
		return
	}
	specificQuestionQuery.Next()
	err = specificQuestionQuery.Scan(&question.ID, &question.Question, &question.CategoryID)
	if err != nil {
		return
	}
	answersQuery, err := db.Query("SELECT ID,answer,correct FROM answers WHERE questionID=?;", question.ID)
	if err != nil {
		return
	}
	for answersQuery.Next() {
		var answer Answer
		err = answersQuery.Scan(&answer.ID, &answer.Answer, &answer.Correct)
		if err != nil {
			return
		}
...
func (q *Question) EditQuestion() (err error) {
	...
	_, err = transaction.Exec("UPDATE questions SET question = ?, categoryID= ? WHERE questions.id = ?;", q.Question, q.CategoryID, q.ID)
	if err != nil {
		_ = transaction.Rollback()
		return
	}
	for _, answer := range q.Answers {
		_, err = transaction.Exec("UPDATE answers SET answer = ?, correct = ? WHERE answers.id = ?", answer.Answer, answer.Correct, answer.ID)
		if err != nil {
			_ = transaction.Rollback()
			return
		}
